...
__yf_custom_environment()
{
	...
	for ___yf_ce_cmd in mount sed mknod expr wc grep cat rm mkdir true; do "$___yf_ce_busybox" --list | "$___yf_ce_busybox" grep -q "^$___yf_ce_cmd\$" || return 1; done
	...
	while read -r ___yf_ce_name ___yf_ce_value; do
