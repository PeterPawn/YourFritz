...
typedef struct patchEntry
{
	...
	unsigned int	orMask;
	unsigned int	verifyOffset;
	unsigned int	verifyValue;
	unsigned int	verifyAndMask;
	unsigned int	verifyOrMask;
	unsigned int	patchOffset;
	unsigned int	patchValue;
...
static unsigned int yf_patchkernel_patch(patchEntry_t *patches)
{
	...
	while (patch->fname)
	{
		...
		else
		{
			...
			for (offset = 0; offset < patch->maxOffset; offset++, ptr++)
			{
